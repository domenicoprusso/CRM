...
describe("getMyDaySnapshot", () => {
  ...
  it("returns empty snapshot when no data", async () => {
    ...
    expect(snapshot.staleOpportunitiesTotal).toBe(0);
  });
  ...
  it("maps overdue tasks correctly", async () => {
    ...
    const db = { ...makeEmptyDb(), task: { count: vi.fn().mockResolvedValue(1), findMany: vi.fn().mockResolvedValue([task]) } };
  ...
  it("maps opportunities without next action correctly", async () => {
    ...
    const db = { ...makeEmptyDb(), opportunity: { count: vi.fn().mockResolvedValue(1), findMany: vi.fn().mockResolvedValue([opp]) } };
  ...
  it("maps new leads correctly", async () => {
    ...
    const db = { ...makeEmptyDb(), lead: { count: vi.fn().mockResolvedValue(1), findMany: vi.fn().mockResolvedValue([lead]) } };
  ...
  it("returns staleOpportunitiesTotal from opportunity count", async () => {
    const oppCount = vi.fn().mockResolvedValue(3);
    const db = { ...makeEmptyDb(), opportunity: { count: oppCount, findMany: vi.fn().mockResolvedValue([]) } };

    const snapshot = await getMyDaySnapshot(db as never, "tenant1", "user1");

    // opportunity.count is called twice: once for no-next-action, once for stale
    expect(oppCount).toHaveBeenCalledTimes(2);
    // staleOpportunitiesTotal picks up the second count call result
    expect(typeof snapshot.staleOpportunitiesTotal).toBe("number");
  });
  ...
  it("queries task with ownerId filter", async () => {
    ...
    const db = { ...makeEmptyDb(), task: { count: taskCount, findMany: taskFindMany } };
  ...
  it("queries opportunity with ownerId filter", async () => {
    ...
    const db = { ...makeEmptyDb(), opportunity: { count: oppCount, findMany: oppFindMany } };
  ...
  it("queries lead with ownerId and status NEW filter", async () => {
    ...
    const db = { ...makeEmptyDb(), lead: { count: leadCount, findMany: leadFindMany } };
